  ...
  try {
    ...
    for (const symbol of symbols) {
      try {
        ...
        // Margin-based exit budget. Take profit at 10% of the margin used and
        // stop out at 50% of the margin used unless Settings say otherwise.
        const marginTakeProfitPercent =
          settings.takeProfitMode === "PERCENT" && Number(settings.takeProfitValue) > 0
            ? Number(settings.takeProfitValue)
            : 10;
        const marginStopLossPercent =
          settings.stopLossMode === "PERCENT" && Number(settings.stopLossValue) > 0
            ? Number(settings.stopLossValue)
            : 50;
        ...
        applyExitOverrides(
          ...
          exitLeverage > 0
            ? {
                ...
                stopLossValue: marginStopLossPercent,
                takeProfitMode: "PERCENT",
                takeProfitValue: marginTakeProfitPercent,
              }
            : settings,
          signalAtr,
          configuredMargin,
        ...
        if (rawStopDistance < minStop && exitLeverage <= 0) {
          logger.warn(
            {
              symbol,
        ...
        try {
          ...
          if (exitLeverage > 0) {
            try {
              ...
              const filled = livePositions.find((p) => p.symbol === symbol && p.size > 0);
              ...
              if (filled && filled.avgPrice > 0) {
                ...
                const levels = computeMarginExitLevels({
                  ...
                  tpMarginPercent: marginTakeProfitPercent,
                  slMarginPercent: marginStopLossPercent,
                  spec,
                });
                ...
                logger.info(
                  {
                    ...
                    takeProfit: levels.takeProfit,
                    stopLoss: levels.stopLoss,
                    targetProfitUsdt: (marginUsed * marginTakeProfitPercent / 100).toFixed(2),
                    maxLossUsdt: (marginUsed * marginStopLossPercent / 100).toFixed(2),
                  },

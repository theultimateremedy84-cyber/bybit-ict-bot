...
import {
  ...
  calculateFixedMarginSize,
  getMarginBasedPriceDistance,
  computeMarginExitLevels,
  getMinTakeProfitDistance,
  getMinStopLossDistance,
  type OpenExposure,
} from "./riskManager";
...
type StopLossMode = "STRATEGY" | "PERCENT" | "ATR";
type TakeProfitMode = "STRATEGY" | "PERCENT" | "R_MULTIPLE";

/**
 * Apply user-configured exits after the strategy has produced a valid signal.
 * The strategy remains the source of truth by default; overrides are only
 * applied when explicitly selected in Settings.
 */
function applyExitOverrides(
  signal: { direction: "BUY" | "SELL"; entryPrice: number; stopLoss: number; takeProfit: number; riskReward: number },
  settings: {
    stopLossMode?: string;
  ...
  const stopMode = (settings.stopLossMode ?? "STRATEGY") as StopLossMode;
  const takeMode = (settings.takeProfitMode ?? "STRATEGY") as TakeProfitMode;
  const stopValue = Number(settings.stopLossValue ?? 0);
  const takeValue = Number(settings.takeProfitValue ?? 0);
  const isBuy = signal.direction === "BUY";
  const useMarginBasedPercent = fixedMarginUsdt > 0 && leverage > 0;

  if (stopMode === "PERCENT" && stopValue > 0) {
    const distance = useMarginBasedPercent
      ? getMarginBasedPriceDistance(signal.entryPrice, stopValue, leverage)
      : signal.entryPrice * (stopValue / 100);
    signal.stopLoss = isBuy ? signal.entryPrice - distance : signal.entryPrice + distance;
  } else if (stopMode === "ATR" && stopValue > 0 && atr > 0) {
    const distance = atr * stopValue;
    signal.stopLoss = isBuy ? signal.entryPrice - distance : signal.entryPrice + distance;
  }

  if (takeMode === "PERCENT" && takeValue > 0) {
    const distance = useMarginBasedPercent
      ? getMarginBasedPriceDistance(signal.entryPrice, takeValue, leverage)
      : signal.entryPrice * (takeValue / 100);
    signal.takeProfit = isBuy ? signal.entryPrice + distance : signal.entryPrice - distance;
  } else if (takeMode === "R_MULTIPLE" && takeValue > 0) {
    const distance = Math.abs(signal.entryPrice - signal.stopLoss) * takeValue;
    signal.takeProfit = isBuy ? signal.entryPrice + distance : signal.entryPrice - distance;
  }

  // ── Economic viability floor ────────────────────────────────────────────
  // Whatever the selected method produced, a target inside the round-trip fee
  // band closes the trade at its own entry price. Widen — never discard.
  const minTp = getMinTakeProfitDistance(signal.entryPrice);
  const minSl = getMinStopLossDistance(signal.entryPrice);
  if (Math.abs(signal.takeProfit - signal.entryPrice) < minTp) {
    signal.takeProfit = isBuy ? signal.entryPrice + minTp : signal.entryPrice - minTp;
  }
  if (Math.abs(signal.entryPrice - signal.stopLoss) < minSl) {
    signal.stopLoss = isBuy ? signal.entryPrice - minSl : signal.entryPrice + minSl;
  }

  const risk = Math.abs(signal.entryPrice - signal.stopLoss);
  signal.riskReward = risk > 0 ? Math.abs(signal.takeProfit - signal.entryPrice) / risk : 0;
}
  ...
  try {
    ...
    for (const symbol of symbols) {
      try {
        ...
        // Load the live contract ceiling before applying margin-based exits.
        // A symbol may support less than the configured account leverage.
        const spec = await getInstrumentSpec(state.client, symbol);
        const configuredMargin = Number(settings.tradeAmountUsdt ?? 7000);
        const configuredLeverage = 50;
        const exitLeverage = configuredMargin > 0
          ? Math.min(configuredLeverage, spec.maxLeverage)
          : 0;

        // Exit methods come straight from Settings. PERCENT is interpreted as
        // a percentage of the margin committed (leveraged), ATR / R_MULTIPLE
        // and STRATEGY keep their own geometry. Nothing is silently replaced.
        const usePercentTakeProfit =
          settings.takeProfitMode === "PERCENT" && Number(settings.takeProfitValue) > 0;
        const usePercentStopLoss =
          settings.stopLossMode === "PERCENT" && Number(settings.stopLossValue) > 0;
        const marginTakeProfitPercent = usePercentTakeProfit ? Number(settings.takeProfitValue) : 0;
        const marginStopLossPercent = usePercentStopLoss ? Number(settings.stopLossValue) : 0;

        // ── Anchor MARKET entries to the live book ───────────────────────
        // The strategy's entryPrice is a projected zone (order block / FVG).
        // A market order does NOT fill there — it fills at the current bid or
        // ask. Deriving TP/SL from the projected price puts one of the levels
        // on the wrong side of the live price, Bybit triggers it the instant
        // the position opens, and the trade closes at its own entry price for
        // two taker fees. That is the "entry price = exit price" loss.
        if (signal.entryStyle === "MARKET") {
          const liveEntry = signal.direction === "BUY" ? ticker.ask1Price : ticker.bid1Price;
          if (liveEntry > 0) signal.entryPrice = liveEntry;
        }

        // Apply the configured exit levels before the signal is logged or
        // persisted. This keeps the dashboard signal, stored trade, position
        // sizing, and Bybit order aligned when the user selects a custom
        // stop-loss or take-profit method.
        const signalAtr = flow?.atr || computeATR(m15Candles, 14);
        applyExitOverrides(
          signal,
          settings,
          signalAtr,
        ...
        if (reverseMode) {
          ...
          // Distances of the ORIGINAL setup. Mirroring these (rather than
          // swapping stop and target outright) is what keeps the counter-trade
          // tradeable: swapping turns a 1:2 into a 1:0.5 and makes the stop as
          // wide as the old target, which starves position sizing until
          // calculateInstitutionalSize rejects the order for being below the
          // exchange minimum — the reason reverse trades never appeared.
          const riskDistance   = Math.abs(originalEntry - originalStopLoss);
          const rewardDistance = Math.abs(originalTakeProfit - originalEntry);

          // The reversed order is filled at market, so anchor everything to the
          // live price. Levels derived from the original pullback entry would
          // sit on the wrong side of the book and Bybit rejects them.
          const marketPrice =
            originalDirection === "BUY" ? ticker.bid1Price : ticker.ask1Price;

          signal.direction  = originalDirection === "BUY" ? "SELL" : "BUY";
          signal.entryStyle = "MARKET";
          signal.entryPrice = marketPrice > 0 ? marketPrice : originalEntry;

          if (signal.direction === "BUY") {
            signal.stopLoss   = signal.entryPrice - riskDistance;
          ...
          signal.riskReward = riskDistance > 0 ? rewardDistance / riskDistance : 0;
          signal.notes = `[REVERSED] Original signal was ${originalDirection} @ ${originalEntry} (stop ${originalStopLoss}, target ${originalTakeProfit}). ${signal.notes ?? ""}`.trim();
          ...
          logger.warn(
            ...
            "🔄 Reverse mode active — executing the mirrored opposite of the generated signal"
          );
        }
        ...
        // Enforce the global open-position limit after every successful order
        // as well as at the start of the scan. openExposures is updated below
        // after each order, so a single scan cannot exceed the configured cap.
        if (openExposures.length >= settings.maxOpenTrades) {
          logger.info(
            { openPositions: openExposures.length, maxOpenTrades: settings.maxOpenTrades },
            "Max open trades reached during scan"
          );
          continue;
        }
        ...
        // Apply the safety floor after user exit overrides. The floor may
        // widen a custom stop, but it must never silently discard a custom
        // percentage target.
        const rawStopDistance = Math.abs(signal.entryPrice - signal.stopLoss);

        if (rawStopDistance < minStop && !usePercentStopLoss) {
          logger.warn(
            {
              symbol,
          ...
          // The mirrored reverse setup keeps the original payoff ratio, so the
          // same minRR floor applies in both modes.
          const floorRR = signal.riskReward > 0 ? signal.riskReward : settings.minRR;
          if (signal.direction === "BUY") {
            signal.stopLoss   = signal.entryPrice - minStop;
            if (settings.takeProfitMode !== "PERCENT") {
        ...
        // Keep R-multiple targets tied to the final, exchange-valid stop.
        if (settings.takeProfitMode === "R_MULTIPLE" && Number(settings.takeProfitValue) > 0) {
          const risk = Math.abs(signal.entryPrice - signal.stopLoss);
          const reward = risk * Number(settings.takeProfitValue);
          signal.takeProfit = roundToTick(
            signal.direction === "BUY" ? signal.entryPrice + reward : signal.entryPrice - reward,
            spec,
          );
        }
        signal.riskReward =
          Math.abs(signal.entryPrice - signal.stopLoss) > 0
        ...
        // Read the value from the same persisted settings row loaded at the
        // beginning of this scan. When it is greater than zero, the selected
        // USDT margin takes precedence over risk-per-trade sizing; the
        // fixed-margin helper converts margin × 50x into base-asset qty.
        const executableQtyCeiling =
          signal.entryStyle === "MARKET"
            ? Math.min(spec.maxOrderQty, spec.maxMarketOrderQty)
            : spec.maxOrderQty;
        const sizeResult =
          configuredMargin > 0
            ? calculateFixedMarginSize({
                marginPerTradeUsdt: configuredMargin,
                accountBalance: balance,
                entryPrice: signal.entryPrice,
                stopLoss: signal.stopLoss,
                spec,
                leverage: configuredLeverage,
                maxOrderQty: executableQtyCeiling,
              })
            : calculateInstitutionalSize({
                accountBalance: balance,
                ...
                stopLoss: signal.stopLoss,
                spec,
                maxLeverage: configuredLeverage,
                confidence: signal.confidence,
        ...
        const portfolioCheck = checkPortfolioRisk({
          ...
          accountBalance: balance,
          openExposures,
          maxPerCorrelationGroup: settings.maxPerCorrelationGroup ?? 5,
          // In fixed-notional mode the selected leverage is the user's
          // deliberate exposure target. The legacy 3x/5x portfolio defaults
          // must not silently reduce a 50x fixed trade to 100k notional.
          maxDirectionalLeverage: configuredMargin > 0
            ? Math.max(Number(settings.maxDirectionalLeverage ?? 3), exitLeverage)
            : (settings.maxDirectionalLeverage ?? 3),
          maxTotalLeverage: configuredMargin > 0
            ? Math.max(Number(settings.maxTotalLeverage ?? 5), exitLeverage)
            : (settings.maxTotalLeverage ?? 5),
        });
        ...
        logger.info(
          {
            ...
            sizingMode: configuredMargin > 0 ? "FIXED_MARGIN" : "RISK_PERCENT",
            orderQtyCeiling: executableQtyCeiling,
        ...
        try {
          if (configuredMargin > 0 && exitLeverage > 0) {
            try {
              await state.client.setLeverage({ symbol, leverage: exitLeverage });
            } catch (leverageErr) {
              const leverageMsg = leverageErr instanceof Error ? leverageErr.message : String(leverageErr);
              // Bybit returns 110043 when the requested leverage is already
              // active. Any other failure is unsafe because the margin maths
              // would no longer describe the actual position.
              if (!/110043|not modified/i.test(leverageMsg)) {
                throw new Error(`Could not set ${exitLeverage}x leverage for ${symbol}: ${leverageMsg}`);
              }
              logger.info({ symbol, leverage: exitLeverage }, "Requested leverage already active");
            }
          }

          // ── Guard: never submit an exit that is already through price ───
          // Re-read the book immediately before sending the order. If either
          // level sits on the wrong side of the live price, submit the entry
          // WITHOUT exits and let the post-fill re-anchor below install exact
          // margin-based TP/SL from the real average price.
          const preTicker = await state.client.getTicker(symbol);
          const livePx = preTicker
            ? (preTicker.bid1Price + preTicker.ask1Price) / 2
            : signal.entryPrice;
          const sideGap = Math.max(spec.tickSize * 2, livePx * 0.0005);
          const exitsValid =
            signal.entryStyle === "LIMIT" ||
            (signal.direction === "BUY"
              ? signal.takeProfit > livePx + sideGap && signal.stopLoss < livePx - sideGap
              : signal.takeProfit < livePx - sideGap && signal.stopLoss > livePx + sideGap);
          ...
          if (!exitsValid) {
            logger.warn(
              ...
              "Exits would trigger immediately — sending a bare entry and attaching TP/SL after the fill"
            );
          }

          const submitStopLoss   = exitsValid ? signal.stopLoss   : undefined;
          const submitTakeProfit = exitsValid ? signal.takeProfit : undefined;
          ...
          // ── Re-anchor TP/SL to the REAL fill ────────────────────────────
          // The levels sent with the order are derived from the signal's
          // projected entry. A market fill (or a limit filled at a better
          // price) lands somewhere else, so those levels can already be
          // through the live price and Bybit closes the position instantly
          // at its own entry price — a pure double-fee loss. Reading the
          // position back and rewriting the stops from the actual average
          // price makes the targets exactly:
          //   take profit =  tpMarginPercent% of the margin used
          //   stop loss   = -slMarginPercent% of the margin used
          if (exitLeverage > 0) {
            try {
              // Poll for the fill: a market order is usually visible within a
              // second, but a busy matching engine can lag. Giving up too
              // early leaves the position without margin-based exits.
              let filled: Awaited<ReturnType<typeof state.client.getPositions>>[number] | undefined;
              for (let attempt = 0; attempt < 8; attempt++) {
                await new Promise((r) => setTimeout(r, 1000));
                const livePositions = await state.client.getPositions(symbol);
                filled = livePositions.find((p) => p.symbol === symbol && p.size > 0);
                if (filled && filled.avgPrice > 0) break;
              }

              if (filled && filled.avgPrice > 0) {
                const liveTicker = await state.client.getTicker(symbol);
                const markPrice = liveTicker
                  ? (liveTicker.bid1Price + liveTicker.ask1Price) / 2
                  : filled.avgPrice;

                // Distances the selected methods produced around the signal
                // entry. Re-applied around the REAL fill so ATR / R-multiple /
                // strategy exits survive slippage exactly like percent exits.
                const plannedTpDistance = Math.abs(signal.takeProfit - signal.entryPrice);
                const plannedSlDistance = Math.abs(signal.entryPrice - signal.stopLoss);

                const levels = computeMarginExitLevels({
                  direction: signal.direction,
                  fillPrice: filled.avgPrice,
                  markPrice,
                  leverage: exitLeverage,
                  tpMarginPercent: usePercentTakeProfit
                    ? marginTakeProfitPercent
                    : (plannedTpDistance / filled.avgPrice) * 100 * exitLeverage,
                  slMarginPercent: usePercentStopLoss
                    ? marginStopLossPercent
                    : (plannedSlDistance / filled.avgPrice) * 100 * exitLeverage,
                  spec,
                });
                ...
                await state.client.setTradingStop({
                  ...
                  positionIdx: filled.positionIdx,
                });

                signal.entryPrice = filled.avgPrice;
                signal.stopLoss   = levels.stopLoss;
                signal.takeProfit = levels.takeProfit;

                const marginUsed = (filled.avgPrice * filled.size) / exitLeverage;
                logger.info(
                  {
                    symbol,
                    fillPrice: filled.avgPrice,
                    ...
                    takeProfit: levels.takeProfit,
                    stopLoss: levels.stopLoss,
                    stopLossMethod: settings.stopLossMode,
                    takeProfitMethod: settings.takeProfitMode,
                    targetProfitUsdt: (
                      Math.abs(levels.takeProfit - filled.avgPrice) * filled.size
                    ).toFixed(2),
                    maxLossUsdt: (
                      Math.abs(filled.avgPrice - levels.stopLoss) * filled.size
                    ).toFixed(2),
                  },
                  "Exits re-anchored to the actual fill (margin-based TP/SL)"
                );
              }
            } catch (stopErr) {
              logger.error(
                { symbol, err: stopErr },
                "Could not re-anchor TP/SL to the fill price — exits remain as submitted"
              );
            }
          }
...
/** Internal signal: trade management is intentionally disabled for this trade. */
class SkipTradeManagement extends Error {}
      ...
      try {
        ...
        if (pos) {
          ...
          try {
            ...
            // Margin-based exits are a fixed budget: moving the stop to
            // break-even converts them into scratch trades that exit at the
            // entry price for two taker fees. Leave them alone.
            const marginExits =
              (settings.stopLossMode === "PERCENT" || settings.takeProfitMode === "PERCENT") &&
              Number(settings.tradeAmountUsdt ?? 0) > 0;
            if (marginExits) {
              throw new SkipTradeManagement();
            }
            ...
            const action = computeTradeManagement({
              ...
              atr,
              spec,
              breakEvenAtR: settings.breakEvenAtR ?? 1.0,
              trailAtR: settings.trailAtR ?? 1.5,
              trailAtrMultiple: settings.trailAtrMultiple ?? 1.5,
              enableTrailing: settings.trailingStop ?? false,
            });
            ...
            if (!(mgmtErr instanceof SkipTradeManagement)) {

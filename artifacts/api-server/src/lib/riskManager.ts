...
// A target that sits closer to the entry than the round-trip taker fee can
// never be profitable: Bybit fills it within seconds and the trade closes at
// (almost) its own entry price. With high leverage a small "percent of margin"
// target collapses to a few ticks — e.g. 0.2% of margin at 50x is a 0.004%
// price move — which is exactly the "open price = exit price" symptom.
// Every exit distance is therefore floored below.

/** Bybit USDT-perp taker fee per side. */
export const TAKER_FEE_RATE = 0.00055;
/** Entry + exit taker cost. */
export const ROUND_TRIP_FEE_RATE = TAKER_FEE_RATE * 2;

/** Minimum take-profit distance: round-trip fees plus a 50% margin of safety. */
export function getMinTakeProfitDistance(price: number, tickSize = 0): number {
  return Math.max(price * ROUND_TRIP_FEE_RATE * 1.5, tickSize * 5);
}

/** Minimum stop-loss distance: never inside the fee + spread noise band. */
export function getMinStopLossDistance(price: number, tickSize = 0): number {
  return Math.max(price * ROUND_TRIP_FEE_RATE, tickSize * 5);
}
...
export function computeMarginExitLevels(input: {
  ...
  const tpDistance = Math.max(
    getMarginBasedPriceDistance(fillPrice, input.tpMarginPercent, leverage),
    getMinTakeProfitDistance(fillPrice, spec.tickSize),
  );
  const slDistance = Math.max(
    getMarginBasedPriceDistance(fillPrice, input.slMarginPercent, leverage),
    getMinStopLossDistance(fillPrice, spec.tickSize),
  );

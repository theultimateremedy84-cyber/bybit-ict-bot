...
export async function analyzeMarket(
  ...
  try {
    ...
    // News blackout — hard block around scheduled macro events
    ...
    // OB recency filter during London manipulation window
    const obCutoff = londonManipulation
      ? new Date(Date.now() - 90 * 60 * 1000)
      : null;
    const orderBlocks = obCutoff
      ? allOrderBlocks.filter((ob) => ob.time < obCutoff)
      : allOrderBlocks;
    ...
    // During London manipulation, block fallback entries — OB or FVG only
    if (londonManipulation && matchingOBs.length === 0 && matchingFVGs.length === 0) {
      return null;
    }

    const bestOB = matchingOBs.sort((a, b) => b.strength - a.strength)[0] ?? null;
    const bestFVG = matchingFVGs.sort((a, b) => Math.abs(b.midpoint - currentBid) - Math.abs(a.midpoint - currentBid))[0] ?? null;
    ...
    if (entryParams.rr < config.minRR) return null;
    ...
    return {
      ...
      htfBias: htfFlow.agreedDirection === "BUY" ? "BULLISH" : "BEARISH",
      structureContext: [

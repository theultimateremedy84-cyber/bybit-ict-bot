...
export class BybitApiClient {
  ...
  /**
   * Set the leverage used by a linear perpetual before a fixed-notional order.
   * The order quantity is independent of this call, but setting it here makes
   * the margin used by the position match the leverage used by TP/SL maths.
   */
  async setLeverage(params: { symbol: string; leverage: number }): Promise<void> {
    const leverage = params.leverage.toString();
    await this.post<Record<string, unknown>>("/v5/position/set-leverage", {
      category: "linear",
      symbol: params.symbol,
  ...
  async getInstrumentInfo(symbol: string): Promise<BybitInstrumentInfo | null> {
    ...
    const maxOrderQty = parseFloat(i.lotSizeFilter.maxOrderQty);
    const parsedMarketMax = parseFloat(i.lotSizeFilter.maxMktOrderQty ?? "");
    ...
    return {
      ...
      maxOrderQty,
      maxMarketOrderQty:
        Number.isFinite(parsedMarketMax) && parsedMarketMax > 0
          ? parsedMarketMax
          : maxOrderQty,

...
const STOP_LOSS_MODES = ["STRATEGY", "PERCENT", "ATR"] as const;
const TAKE_PROFIT_MODES = ["STRATEGY", "PERCENT", "R_MULTIPLE"] as const;
...
const updateSettings = async (req: Request, res: Response) => {
  try {
    ...
    if (body.tradeAmountUsdt !== undefined) {
      const value = Number(body.tradeAmountUsdt);
      if (!Number.isFinite(value) || value < 0 || value > 1_000_000) {
        res.status(400).json({ error: "tradeAmountUsdt margin must be between 0 and 1000000 USDT" });
        return;
      }
      updateData.tradeAmountUsdt = value;
    }
    if (body.riskPerTrade       !== undefined) updateData.riskPerTrade       = body.riskPerTrade;
    if (body.maxOpenTrades !== undefined) {
      const value = Number(body.maxOpenTrades);
      if (!Number.isInteger(value) || value < 1 || value > 20) {
        res.status(400).json({ error: "maxOpenTrades must be an integer between 1 and 20" });
        return;
      }
      updateData.maxOpenTrades = value;
    }
    ...
    if (body.stopLossMode !== undefined) {
      if (!STOP_LOSS_MODES.includes(body.stopLossMode as (typeof STOP_LOSS_MODES)[number])) {
        res.status(400).json({ error: "stopLossMode must be STRATEGY, PERCENT, or ATR" });
        return;
      }
      updateData.stopLossMode = body.stopLossMode;
    }
    if (body.stopLossValue !== undefined) {
      const value = Number(body.stopLossValue);
      if (!Number.isFinite(value) || value < 0 || value > 100) {
        res.status(400).json({ error: "stopLossValue must be between 0 and 100" });
        return;
      }
      updateData.stopLossValue = value;
    }
    if (body.takeProfitMode !== undefined) {
      if (!TAKE_PROFIT_MODES.includes(body.takeProfitMode as (typeof TAKE_PROFIT_MODES)[number])) {
        res.status(400).json({ error: "takeProfitMode must be STRATEGY, PERCENT, or R_MULTIPLE" });
        return;
      }
      updateData.takeProfitMode = body.takeProfitMode;
    }
    if (body.takeProfitValue !== undefined) {
      const value = Number(body.takeProfitValue);
      if (!Number.isFinite(value) || value < 0 || value > 100) {
        res.status(400).json({ error: "takeProfitValue must be between 0 and 100" });
        return;
      }
      updateData.takeProfitValue = value;
    }
    ...
    if (body.maxLeverage !== undefined) {
      const value = Number(body.maxLeverage);
      if (!Number.isFinite(value) || value < 1 || value > 100) {
        res.status(400).json({ error: "maxLeverage must be between 1 and 100" });
        return;
      }
      updateData.maxLeverage = value;
    }

...
export default function Dashboard() {
  ...
  const { data: account, isLoading: accountLoading } = useGetAccount({ query: { refetchInterval: 10000, queryKey: getGetAccountQueryKey() } });
  const { data: positions, isLoading: positionsLoading } = useGetPositions({ query: { refetchInterval: 5000, queryKey: getGetPositionsQueryKey() } });
            ...
            <div className="flex justify-between items-start">
              <div className="space-y-2">
                ...
                {accountLoading ? (
                  ...
                  <p className={`text-2xl font-bold font-mono ${account && account.unrealisedPnl >= 0 ? 'text-success' : 'text-destructive'}`}>
                    {account && account.unrealisedPnl > 0 ? '+' : ''}{formatCurrency(account?.unrealisedPnl)}
                  </p>
                )}
              </div>

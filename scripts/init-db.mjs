...
try {
  ...
  // ── Institutional order-flow upgrade (idempotent) ────────────────────────
  const alters = [
    ...
    `UPDATE bot_settings SET stop_loss_mode = 'PERCENT', stop_loss_value = 50 WHERE stop_loss_mode = 'STRATEGY' AND stop_loss_value <= 0`,
    `UPDATE bot_settings SET take_profit_mode = 'PERCENT', take_profit_value = 10 WHERE take_profit_mode = 'STRATEGY' AND take_profit_value <= 0`,
    // One-time forced reset: TP = 10% of margin (700 USDT on 7,000),
    // SL = 50% of margin (3,500 USDT on 7,000). Runs once per database.
    `ALTER TABLE bot_settings ADD COLUMN IF NOT EXISTS margin_exit_defaults_version INTEGER NOT NULL DEFAULT 0`,
    `UPDATE bot_settings SET stop_loss_mode = 'PERCENT', stop_loss_value = 50, take_profit_mode = 'PERCENT', take_profit_value = 10, margin_exit_defaults_version = 1 WHERE margin_exit_defaults_version < 1`,
  ...
  console.log("  \u2713 order-flow columns");

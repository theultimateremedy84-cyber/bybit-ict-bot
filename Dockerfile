...
# Frontend (lib/db contains the @workspace/api-client-react package)
...
# 1. Build the React dashboard  →  artifacts/dashboard/dist/public/
RUN pnpm --filter @workspace/dashboard run build

# 2. Build the Express API bundle  →  artifacts/api-server/dist/
RUN pnpm --filter @workspace/api-server run build

# 3. Copy the React build into dist/public/ so Express can serve it
#    Vite outputs to artifacts/dashboard/dist/public/ — copy that directly
RUN cp -r artifacts/dashboard/dist/public/. artifacts/api-server/dist/public/
